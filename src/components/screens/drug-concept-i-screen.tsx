...
import type { DrugSynthesizedAnswer } from "@/data/drug-monograph";
...
function getAnswerKey(question: string): string {
  ...
  if (
    ...
    return "renal-dose-gfr35";
  }
  return "afib-dose";
}

function buildReferencesFromAnswer(answer: DrugSynthesizedAnswer): AiAnswerReference[] {
  return answer.citations.map((citation) => {
    const sf = getSubfieldById(apixabanMonograph, citation.anchor);
    return {
      detail: sf?.summary ?? "",
      id: citation.marker,
...
export function DrugConceptAccordionTabsScreen() {
  ...
  const [canvasAnchor, setCanvasAnchor] = useState<string | undefined>(undefined);
  ...
  const openCanvasAt = useCallback((subfieldId: string) => {
    setCanvasAnchor(subfieldId);
    setCanvasOpen(true);
  }, []);
  ...
  const submitQuestion = useCallback(
    (question: string) => {
      ...
      const key = getAnswerKey(trimmed);
      const synthesizedAnswer = apixabanMonograph.synthesizedAnswers[key];
      const references = buildReferencesFromAnswer(synthesizedAnswer);
      const matchedSubfieldId = getMatchedSubfieldId(trimmed);

      setTurns((prev) => [
        ...prev.map((t): ChatTurn => ({ ...t, status: "complete" })),
        { id, matchedSubfieldId, question: trimmed, references, status: "preparing", synthesizedAnswer },
      ]);
      setDraft("");
  ...
  return (
    <DrugConceptShell activeConcept="I">
      <div className="relative flex min-h-0 flex-1">
        {/* Chat / answer column — shrinks on desktop when canvas is open */}
        <section
          className={[
            "relative flex min-h-0 flex-col",
            canvasOpen
              ? "flex-1 md:w-[48%] md:flex-none md:min-w-[360px]"
              : "flex-1",
          ].join(" ")}
        >
          {/* Top fade */}
              ...
              {turns.length === 0 ? (
                <div className="flex flex-col items-center py-12 md:py-20">
                  ...
                  <h2 className="dc-rise mt-2 max-w-[380px] text-center text-[23px] font-extrabold leading-[1.2] tracking-[-0.02em] text-[#161b1d] [text-wrap:balance] md:text-[29px]">
                    Answer Tabs + Accordion Drug Info
                  </h2>
                  <p className="dc-rise mt-3 max-w-[430px] text-center text-[14px] leading-[1.7] text-[#5a6e7e] [text-wrap:balance]">
                    Combines E and C: Answer · Drug Information · References tabs on every reply,
                    but the Drug Information tab shows the full progressive accordion. "Full section"
                    links open the complete monograph canvas alongside.
                  </p>
                  ...
                  <article key={turn.id} className="mx-auto mb-10 max-w-[860px] last:mb-0">
                    ...
                    {turn.status === "preparing" ? (
                      <AiPreparingAnswerNotice
                        ...
                        text="Synthesizing drug reference…"
                      />
                    ) : (
                      <DrugAnswerTabs
                        drugInfoMode="accordion"
                        monograph={apixabanMonograph}
                        onOpenMonograph={openCanvasAt}
                        question={turn.question}
                        references={turn.references}
                        synthesizedAnswer={turn.synthesizedAnswer}
                        initialAccordionAnchor={turn.matchedSubfieldId}
                        analyticsContext={{
