...
import { apixabanMonograph, getMatchedTaskChipId } from "@/data/drug-monograph";
...
type ChatTurn = {
  id: number;
  initialTaskChipId: string | undefined;
  question: string;
  status: TurnStatus;
};
...
const SUGGESTED_QUERIES = [
  ...
  "Apixaban DVT treatment",
  "Apixaban drug interactions",
];

export function DrugConceptWorkflowChipsScreen() {
  ...
  const submitQuestion = useCallback(
    (question: string) => {
      ...
      // Auto-route to matching chip; bare "apixaban" gets no match → defaults to afib-dosing in the card
      const initialTaskChipId = getMatchedTaskChipId(trimmed);

      setTurns((prev) => [
        ...prev.map((t): ChatTurn => ({ ...t, status: "complete" })),
        { id, initialTaskChipId, question: trimmed, status: "preparing" },
      ]);
      setDraft("");
  ...
  return (
    <DrugConceptShell activeConcept="D">
      <section className="relative flex min-h-0 flex-1 flex-col">
        ...
        <div ref={scrollRef} className="min-h-0 flex-1 overflow-y-auto overscroll-contain">
          ...
          >
            {turns.length === 0 ? (
              /* Landing state — never a gate, always inviting */
              <div className="flex flex-col items-center py-12 md:py-20">
                ...
                <p className="dc-rise mt-3 max-w-[440px] text-center text-[14px] leading-[1.7] text-[#5a6e7e] [text-wrap:balance]">
                  Type a drug name for a useful default card, then tap a clinical task chip
                  to reorganize the answer around your goal. Context-aware queries
                  auto-route to the right task.
                </p>
              ...
              turns.map((turn) => (
                <article key={turn.id} className="mx-auto mb-10 max-w-[860px] last:mb-0">
                  ...
                  {turn.status === "preparing" ? (
                    ...
                    <DrugWorkflowCard
                      initialTaskChipId={turn.initialTaskChipId}
          ...
          <div className="mx-auto w-full max-w-[900px] px-4 pb-0 md:px-6">
            <div className="rounded-t-[28px] bg-gradient-to-b from-transparent via-white/82 to-white px-2 pb-[max(env(safe-area-inset-bottom),6px)] pt-3 md:pt-4">
              <AiResponseChatComposer
                analyticsSourceSurface="drug_concept_d"
